...
//    final static String _url  = "jdbc:mysql://172.28.146.165:3306/sakurai_automation";
	...
	public static HashMap<String, String> getAnswersForMLQuiz(String quizId)
			throws SQLException, ClassNotFoundException {
		final String query = "select distinct q.question_text, a.text, (a.correct_response_id is not null) correct from "
				+ "answer_choices a, " + "interactions i, " + "quiz_questions qq, " + "questions q " + "where" + " "
				+ "qq.quiz_id = " + quizId + " and" + " " + "qq.question_id = q.id and" + " "
				+ "qq.question_id = i.question_id and" + " " + "a.interaction_id = i.id and" + " "
				+ "a.correct_response_id is true" + " " + "order by q.question_text";
		HashMap<String, String> questionAnswers = new HashMap<String, String>();
		Class driverClass = Class.forName("com.mysql.jdbc.Driver");
		Connection jdbcConnection = DriverManager.getConnection(_url, "automation_user", "automationpass");
...
//    System.out.println("questions=="+ questions);
		...
		String q = "";
		// File f = new File("C:\\Users\\rohitsingh\\Desktop\\automation-fixed.sql"); //
		// source path is the absolute path of dumpfile.
		try {
		...
		// We must separate the queries, so they can be executed. And Java Simply does
		// this:
...
//          }
	...
	public static HashMap<String, String> getAnswersForPracticeExam(String quizId)
			throws SQLException, ClassNotFoundException {
		final String query = "select distinct q.question_text, a.text, (a.correct_response_id is not null) correct from "
				+ "answer_choices a, " + "interactions i, " + "nclex_exam_questions eq, " + "questions q " + "where"
				+ " " + "eq.nclex_exam_id = " + quizId + " and" + " " + "eq.question_id = q.id and" + " "
				+ "eq.question_id = i.question_id and" + " " + "a.interaction_id = i.id and" + " "
				+ "a.correct_response_id is true" + " " + "order by q.question_text";

		final String query1 = "SELECT q.question_text, a.text, (a.correct_response_id IS NOT NULL) correct FROM "
				+ "answer_choices a, " + "interactions i, " + "quiz_questions qq, " + "questions q "
				+ "WHERE qq.quiz_id = " + quizId + " AND" + " qq.question_id = q.id AND "
				+ "qq.question_id = i.question_id AND " + "a.interaction_id = i.id AND "
				+ "a.correct_response_id IS TRUE " + "ORDER BY q.question_text";

		HashMap<String, String> questionAnswers = new HashMap<String, String>();
		Class driverClass = Class.forName("com.mysql.jdbc.Driver");
		Connection jdbcConnection = DriverManager.getConnection(_url, "sumita", "sumita123");
...
//    }
	...
	public static ArrayList<String> fetchCredentials(String DB_Name, String User_Name,String Password,String Host,String Port,String Symbol_Text) {
