...
public class YamlReader {
	...
	public static String setYamlFilePath() {
		...
		if (tier.equalsIgnoreCase("dev")) {
			...
			yamlFilePath = "src/test/resources/testdata/E2E_TestData.yml";
	...
	public static String setTestDataYamlPath(String pcode)
	{
		String Product=pcode;
		if (Product.equalsIgnoreCase("AP_Chemistry")) {
			yamlFilePath = "src/test/resources/testdata/AP_Chemistry_TestData.yml";
		} else if (Product.equalsIgnoreCase("AP-US-History")) {
		yamlFilePath = "src/test/resources/testdata/AP_US_History_TestData.yml";
		
		} else {
			Reporter.log(
					"YOU HAVE PROVIDED WRONG Product code,",	true);
		}
		return yamlFilePath;
